...
public class Exercises4{
	...
	public void init(int count){//初始化所有算式count代表算式的个数
		for(int i=0;i<count;i++){
			...
			while(true){
				...
				for(int j=0;j<i;j++){
					...
					if(equs[i].getAnswer()>100||equs[i].getAnswer()<0){//判断答案是否在0到100之间(含0和100)
						flag=1;
					}
					if(flag==0)//式子合法进行下一个
						break;
			...
			while(true){
				...
				for(int j=0;j<i;j++){
					...
					if(flag==0)//式子合法进行下一个
						...
						equs[i].init();//如果式子不合法重新生成式子
				}
			}
	
	}
	
	public void printAll(){//输出所有的合法式子
		for(int i=0;i<50;i++){
			System.out.format("%10s",equs[i].getMe());
		}
	}
	public void printAll(int turn){//输出所有的式子 每行输出turn个式子
		for(int i=0;i<50;i++){
	...
	public void printAnswer(int turn){//输出所有答案 每行输出turn个答案
		for(int i=0;i<50;i++){
			if(turn>0)if(i%turn==0)System.out.print("\n");
				System.out.format("% 4d",equs[i].getAnswer());
		}
